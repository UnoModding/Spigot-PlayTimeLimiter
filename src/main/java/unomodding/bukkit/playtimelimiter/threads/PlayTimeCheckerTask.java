...
public class PlayTimeCheckerTask extends TimerTask
{
    ...
    public void run()
    {
        for (Player player : this.plugin.getServer().getOnlinePlayers()) {
            int timeLeft = this.plugin.getTimeAllowedInSeconds(player.getUniqueId());
            if (timeLeft <= 0) {
                ...
                player.kickPlayer("You have exceeded the time allowed to play! Come back in "
                        ...
                        + "!");
            } else if (timeLeft <= 10 && !this.plugin.hasPlayerSeenMessage(player.getUniqueId(), 10)) {
                ...
                this.plugin.sentPlayerWarningMessage(player.getUniqueId(), 10);
                    ...
                    && !this.plugin.hasPlayerSeenMessage(player.getUniqueId(), 60)) {
                player.sendMessage(ChatColor.RED
                        + "WARNING!"
                        + ChatColor.RESET
                ...
                this.plugin.sentPlayerWarningMessage(player.getUniqueId(), 60);
                    ...
                    && !this.plugin.hasPlayerSeenMessage(player.getUniqueId(), 300)) {
                player.sendMessage(ChatColor.RED
                        + "WARNING!"
                        + ChatColor.RESET
